...
class UserController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->orderby)){
			...
			$db->orderBy("user.npk", ORDER_TYPE);
		}
		if($fieldname){
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("user.npk", $rec_id);; //select record based on primary key
		}
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("npk","nama","alamat","jenis_kelamin","email","password","images","role");
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("npk","nama","alamat","jenis_kelamin","email","images");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'npk' => 'required',
			...
			if(isset($modeldata['nama'])){
				$db->where("nama", $modeldata['nama'])->where("npk", $rec_id, "!=");
				if($db->has($tablename)){
					$this->view->page_error[] = $modeldata['nama']." Already exist!";
				}
			}
			...
			if(isset($modeldata['email'])){
				$db->where("email", $modeldata['email'])->where("npk", $rec_id, "!=");
				if($db->has($tablename)){
					$this->view->page_error[] = $modeldata['email']." Already exist!";
				}
			} 
			if($this->validated()){
				$db->where("user.npk", $rec_id);;
	...
	function delete($rec_id = null){
		...
		$db->where("user.npk", $arr_rec_id, "in");
